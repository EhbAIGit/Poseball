...
class client_messenger:
    ...
    def connect_client(self):
        try:
            ...
            send_message = "CLIENT 1 CONNECTED"
        ...
        while (self.CLIENT.MODE == "ESTABLISH TOY"):
            ...
            if (received_message == "BEGIN TOY CONNECTION"):
                ...
                try:
                    ...
                    self.CLIENT.send_message.put("TOY DISCONNECT")
                    self.CLIENT.MODE = "TOY DISCONNECT"
                    break
            elif received_message is not None:
                print(f"Wrong command received. reinitiating toy connection")
                oelf.CLIENT.send_message.put("TOY DISCONNECT")
                self.CLIENT.MODE = "TOY DISCONNECT"
                break
    ...
    def state_machine(self):
        with self.connect_toy() as api:
            ...
            while exit_condition is False:
                if self.CLIENT.received_message.empty() is False:
                    received_message = self.CLIENT.received_message.get()
                    if(received_message == "ESTABLISH TOY") or (received_message == "GAME STOP"):
                ...
                if(self.CLIENT.MODE == "TOY DISCONNECT"):
                    ...
                    self.CLIENT.send_message.put("WAITING TO START")
                    ...
                    if(received_message == "GAME OVER"):
                        ...
                        self.CLIENT.send_message.put("WAITING TO RESTART")
                else:
                    time.sleep(1)
                    continue
                
                self.timer = time.time() - timer_start
                exit_condition = (self.timer > self.timer_limit)
                
                self.battery_voltage = Power.get_battery_voltage(self.toy)
                if (self.battery_voltage < self.voltage_threshold):
                    self.CLIENT.send_message("TIMER OVER")
                ...
                if(lmList is not None and (len(lmList) > 30)):
                    ...
                    if self.server_forced_move:
                        ...
                        if(self.MODE == "CALIBRATION BEFORE START"):
                            ...
                            self.CLIENT.send_message.put("FIRST CALIBRATION OVER")
...
def main():
    ...
    if(CLIENT.MODE == "CLIENT CONNECTED"):
        ...
        while (True):
            ...
            del Sphero_obj
            CLIENT.send_message.put("TOY DISCONNECT")
            CLIENT.MODE = "ESTABLISH TOY"
